...
进入编辑器后，点击 **Config**，选择 OpenAI、OpenRouter 或火山引擎，并填写 API key、模型名称和兼容 Chat Completions 的 Base URL。API key 只存储在浏览器 `sessionStorage` 的 `prologue.aiConfig.v1` 下，只会随当前请求发送到后端，应用不会持久化存储。
