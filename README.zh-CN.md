...
- 实时 A4 简历预览，并支持浏览器打印和服务端渲染 PDF 导出。
...
- 通过 OpenAI 兼容接口支持 OpenAI 和 OpenRouter 的 AI 简历文本润色。
...
- **打印导出：** react-to-print，Node PDF 流式导出使用 Playwright
...
- Node.js
- npm
- 可选的 OpenAI 或 OpenRouter API key，用于 AI 润色和 AI 导入功能
...
进入编辑器后，在 **Design / AI Service** 中选择 OpenAI 或 OpenRouter，并填写 API key 与模型名称。API key 只存储在浏览器 `sessionStorage` 的 `prologue.aiConfig.v1` 下，只会随当前请求发送到后端，应用不会持久化存储。
...
编辑器保留浏览器打印作为通用 fallback。**PDF** 按钮会调用 `app/api/export/pdf/route.ts`，该 route 使用 Node runtime 和 Playwright 流式返回 `application/pdf` 下载文件。

该能力需要支持 Node/Chromium 的部署环境。Cloudflare Worker 部署应继续使用浏览器打印，除非额外接入独立的 Node PDF 服务。
